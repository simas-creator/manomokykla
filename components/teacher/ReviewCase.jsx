...
const ReviewCase = ({ review }) => {
  ...
  return (
    <div
      className={`border border-black rounded-xl px-4 ${
        !comment
          ? "h-fit sm:w-78 w-[80%] bsm:w-fit max-w-md"
          : "max-w-md h-fit w-[85%] sm:w-fit sm:w-90"
      }`}
    >
      <div className="flex items-center justify-between pt-3 flex-wrap gap-2">
        ...
        {rec === true ? (
          ...
          <div className="w-7 h-7 pt-1">
            <img src="/images/thumbs-down-red.svg" alt="" />
          </div>
        )}
      </div>
      {comment && <div className="line-clamp-3 mt-3 text-lg">{comment}</div>}
      ...
      <div className="pt-4 pb-4 flex justify-between items-center flex-wrap gap-6">
        ...
        <div className="">
          <button
            ...
            className="px-5 py-2 btn btn-primary btn-outline"
          >
            Išsamiau
          </button>

...
const SchoolForm = () => {
  ...
  useEffect(() => {
    if (session?.user?.id) {
      setJsonData((prev) => ({ ...prev, user: session.user.id }));
    }
  }, [session]);
  ...
  const handleSubmit = async (e) => {
    ...
    if(!isValidName.ok) {
      setLoading(false);
      setError("Mokykla jau egzistuoja");
      return;
    }
  ...
  return (
    <section className="flex justify-center flex-col px-4 py-8 max-w-3xl mx-auto">
      <h1 className="text-center text-3xl font-title text-gray-800 mb-6 mt-4 font-semibold">
        Pridėti mokyklą
      </h1>
      <form
        onSubmit={handleSubmit}
        className="bg-white shadow-lg border border-primary rounded-lg p-8 space-y-6 font-title"
      >
        {/* Input for Name */}
        ...
        <div className="flex items-center space-x-2">
          <label
            htmlFor="files"
            className="btn btn-outline btn-primary px-4 py-2 rounded-lg"
          >
